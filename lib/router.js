...
/** NVIDIA_API_KEYS may be a JSON array, CSV, or a single nvapi- key. */
export function parseNvidiaApiKeys(raw) {
  ...
  const cleaned = parts.map((k) => k.trim().replace(/^["']|["']$/g, '')).filter(Boolean);
  const nv = cleaned.filter((k) => k.startsWith('nvapi-'));
  return nv.length ? nv : cleaned;
}
