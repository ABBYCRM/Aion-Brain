...
import { defaultResearch, RESEARCH_META } from './research.js';
 ...
 * 1. Run full Auditor
 * 2. For each P0/P1 finding, attempt external research (defaults to DuckDuckGo HTML; pass researchFn to override)
 * 3. Produce structured proposals only (never auto-apply by default)
 * 4. Re-audit after optional apply
 */
export class Brain {
  constructor({ root, store, researchFn, enableResearch = true } = {}) {
    this.root = root || process.cwd();
    this.store = store || null;
    // Default to the real DuckDuckGo research; can be overridden (e.g. in tests).
    this.researchFn = researchFn || (enableResearch ? defaultResearch : defaultResearchStub);
  }
  ...
  async runCycle({ apply = false, severities = ['P0', 'P1'] } = {}) {
    ...
    const result = {
      ...
      research_provider: this.researchFn === defaultResearch ? RESEARCH_META.provider : 'custom',
    };
  ...
  async _researchFinding(finding) {
    // Keep the query short and natural-language; `site:` operators and overly
    // long rule codes make DDG's HTML endpoint return nothing. Strip the rule
    // prefix and the boilerplate "node.js express fix" — the message itself
    // already says "in node.js".
    const cleaned = String(finding.message || '')
      .replace(/^P[0-2]-[\w-]+:?\s*/i, '')
      .replace(/\s+/g, ' ')
      .trim()
      .slice(0, 120);
    const query = `${cleaned} node.js best practice`;
    try {
