...
class AionSettings {
  constructor() {
    ...
    // v0.1.13 — operator said "use NVIDIA as the main LLM and a NVIDIA fast
    // model as the reranker." Primary is NVIDIA's fast 8B chat model
    // (NVIDIA-first deployment). Fallbacks: NVIDIA reasoning, Grok fast
    // reasoning, OpenAI mini, Anthropic Haiku. Bare model IDs (not
    // "provider/model") except NVIDIA, whose own catalog IDs are genuinely
    // namespaced ("nvidia/nemotron-..."). See aion_chain.js's
    // resolveModelRef() for how these map to a configured provider.
    //
    // v0.1.14 — meta/llama-3.1-8b-instruct was retired by NVIDIA on
    // 2026-08-26T09:00:00Z (HTTP 410 from integrate.api.nvidia.com).
    // meta/llama-3.3-70b-instruct is also 410. Promoted the existing
    // nvidia/nemotron-3-nano-30b-a3b fallback (verified HTTP 200) to
    // primary; new fallbacks: bigger NVIDIA reasoning, Grok, OpenAI,
    // Anthropic — same shape as before, just re-ordered around a
    // model that the upstream actually serves.
    this.primaryModel = fromEnv('PRIMARY_MODEL', 'nvidia/nemotron-3-nano-30b-a3b');
    this.fallbackModels = csv(fromEnv('FALLBACK_MODELS', 'meta/llama-3.3-70b-instruct,grok-4-fast-reasoning,gpt-4.1-mini,claude-3-5-haiku-latest'));
