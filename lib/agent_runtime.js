...
export function createLlmPlanner({ chain, catalog, onEvent, model } = {}) {
  ...
  return async function llmPlanner(state, control) {
    ...
    if (extracted.error) state.warnings.push(extracted.error);
    ...
    if (text) return { type: 'respond', strategy: 'respond', text };
...
function pickForcedTool(state, control) {
  ...
  if (/\b(code|coding|implement|debug|repair|python|javascript|typescript|fastapi|pandas|selenium|firecrawl)\b/i.test(goal)) {
    prefer.push('teacher_rag_teach');
  }
...
function buildAgentMessages(state, control, lastReasoning) {
  ...
  const system = [
    ...
    'For coding tasks, teacher_rag_teach retrieves grounded implementation knowledge. When the goal explicitly requires creating or repairing runnable code, teacher_rag_build can create files, execute code, repair failures, and returns success only after fresh passing tests.',
  ...
  if (lastReasoning) {
    messages.push({ role: 'assistant', content: '', reasoning_content: lastReasoning });
    messages.push({ role: 'user', content: 'Your previous turn was reasoning only. Take an ACTION now: call a tool.' });
  }
  messages.push({ role: 'user', content: `Active goal: ${snap.active_goal}\nExecute the next ACTION.` });
  return messages;
}
...
export class AgentRuntime {
  ...
  async run({
    ...
    const planner = createLlmPlanner({ chain: this.chain, catalog, onEvent, model });
    ...
    if (typeof onEvent === 'function') {
      onEvent({ type: 'runtime_done', status: result.status, verified: result.verified, cycles: result.cycles.length });
    }
    return result;
  }
}
