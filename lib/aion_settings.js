...
class AionSettings {
  constructor() {
    ...
    // v0.1.12 — operator said "I need a stronger model than chat gpt mini"
    // and named nvidia + xai as available alternatives. Bare model IDs
    // (not "provider/model") except NVIDIA, whose own catalog IDs are
    // genuinely namespaced ("nvidia/nemotron-..."). See aion_chain.js's
    // buildModelChain() for how these map to a configured provider.
    this.primaryModel = fromEnv('PRIMARY_MODEL', 'gpt-5');
    this.fallbackModels = csv(fromEnv('FALLBACK_MODELS', 'gpt-4.1,nvidia/nemotron-3-super-120b-a12b,grok-4'));
