...
export class AionChain {
  ...
  async *stream({ chain = [], messages, temperature = 0.7, maxTokens = 1024 }) {
    // NOTE: This is *simulated* streaming. The underlying OpenAIProvider /
    // AnthropicProvider currently return the full completion, which is then
    // chunked into SSE delta events for UI compatibility. True token-level
    ...
    // Name-based provider selection: walk `order` (the requested chain) and
    // resolve each entry by name. This lets callers request a specific
    // provider regardless of its position in this.providers.
    ...
    for (let i = 0; i < requestedOrder.length; i++) {
      ...
      try {
        ...
        const onDelta = (text) => {
          if (!firstOpened) {
            firstOpened = true;
            // open event is emitted synchronously on first chunk
          }
        };

        // Call the provider. OpenAIProvider returns full text (no streaming);
        // we wrap it in a single delta so the SSE shape matches the Python
        // backend's stream_chat output.
        const result = await provider.invoke({
          operation: 'chat',
          payload: { model: ref.model, messages, temperature, max_tokens: maxTokens },
        });
        ...
        yield sseEvent('open', { provider: ref.provider, model: ref.model });
        // Stream character-by-character to mimic the Python streaming
        // (the underlying provider gives us the full text; chunked SSE
        // events make the UI feel live)
        const chunkSize = 12;
        ...
        this._record({ kind: 'attempt_ok', provider: ref.provider, model: ref.model, latency_ms: latency, completion_chars: content.length });
        return;
      } catch (e) {
        const latency = Date.now() - started;
