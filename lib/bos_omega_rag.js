...
function embeddingsConfigured() {
  return Boolean(
    String(process.env.EMBEDDINGS_API_KEY || '').trim()
    || String(process.env.BITDEER_API_KEY || '').trim()
    || String(process.env.BITDEER_API_KEYS || '').trim()
    || String(process.env.NVIDIA_API_KEY || '').trim()
    || String(process.env.NVIDIA_API_KEYS || '').trim(),
  );
}
...
export class BosOmegaRag {
  ...
  upsertDocument({ sourceId, title, content, authority } = {}) {
    ...
    tx(rows);
    return { ok: true, source_id: sid, authority: auth, chunk_count: rows.length, upserted: true };
  }
  ...
  async retrieveWithOptionalRemote(query, opts = {}) {
    ...
    try {
      const remote = await this._pineconeQuery(hashEmbed(query), opts.topK || 6);
      if (!remote.length) return { ...local, pinecone: 'empty' };
  ...
  async _pineconeQuery(vector, topK) {
    const host = String(process.env.PINECONE_INDEX_HOST || '').replace(/^https?:\/\//, '').replace(/\/+$/, '');
    const key = String(process.env.PINECONE_API_KEY || '').trim();
    if (!host || !key) return [];
    const res = await this.fetchImpl(`https://${host}/query`, {
      method: 'POST',
      headers: {
        'Api-Key': key,
      ...
      body: JSON.stringify({
        ...
        namespace: process.env.PINECONE_NAMESPACE || 'bos-omega',
      }),
      signal: AbortSignal.timeout(15_000),
    });
    if (!res.ok) throw new Error(`pinecone_http_${res.status}`);
