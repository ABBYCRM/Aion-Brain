...
const CONNECTORS = Object.freeze([
  { name: 'bitdeer', kind: 'inference', env: ['BITDEER_API_KEY', 'BITDEER_API_KEYS'], when: 'Default Brain chat / rerank / images via Bitdeer. Use when a Bitdeer model should answer.' },
  { name: 'nvidia', kind: 'inference', env: ['NVIDIA_API_KEY', 'NVIDIA_API_KEYS'], when: 'Bitdeer alias keys. Same chain as bitdeer.' },
  { name: 'openai', kind: 'inference', env: ['OPENAI_API_KEY'], when: 'Use openai_chat / openai_embed when the operator wants OpenAI directly. Chat /v1 stays Bitdeer.' },
  ...
  { name: 'gemini', kind: 'inference', env: ['GEMINI_API_KEY'], when: 'Use gemini_chat for Gemini generateContent. Fail-soft if GEMINI_API_KEY is missing.' },
  { name: 'xai', kind: 'inference', env: ['XAI_API_KEY'], when: 'Use xai_chat for Grok. Key is snapshotted before the Bitdeer-only /v1 guard strips process.env.' },
  { name: 'kimi', kind: 'inference', env: ['KIMI_API_KEY'], when: 'Use kimi_chat for Moonshot/Kimi. Fail-soft if KIMI_API_KEY is missing.' },
